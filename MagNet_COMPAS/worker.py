...
from setup_mnist import COMPAS
...
class SimpleReformer:
    ...
    def heal(self, X):
        ...
        return np.clip(X, -21.14031684, 37.71382819)
...
class Operator:
    def __init__(self, data, classifier, det_dict, reformer):
        ...
        self.normal = self.operate(AttackData(self.data.test_group_data,
                                              np.argmax(self.data.test_group_pred_labels, axis=1), "Normal"))
    ...
    def operate(self, untrusted_obj):
        ...
        return np.array(list(zip(np.array(Y_judgement).flatten(), np.array(Y_prime_judgement).flatten())))
...
class Evaluator:
    ...
    def get_normal_acc(self, normal_all_pass, num_samples):
        """
        ...
        predicted_filtered = [1 if XpC else 0 for _, XpC in filtered_normal_tups]
        predicted = np.zeros(num_samples)
        f_idx = 0
        for idx in normal_all_pass:
            predicted[idx] = predicted_filtered[f_idx]
            f_idx += 1

        both_acc = sum(predicted) / num_samples
        det_only_acc = sum(1 for XC, XpC in filtered_normal_tups if XC) / num_samples
        ref_only_acc = sum([1 for _, XpC in normal_tups if XpC]) / num_samples
        none_acc = sum([1 for XC, _ in normal_tups if XC]) / num_samples
    ...
    def get_attack_acc(self, attack_pass):
        ...
        print('Num untrusted: ', num_untrusted)
    ...
    def plot_various_confidences(self, graph_name, drop_rate,
        ...
        all_pass, _ = self.operator.filter(self.operator.data.test_group_data, thrs)
        all_on_acc, _, _, _, predicted = self.get_normal_acc(all_pass, 700)
        ...
        print("Classification f1 with all defense on - all: ",
              f1_score(np.argmax(data.test_group_pred_labels, axis=1), predicted))
        labels = data.test_group_detc_labels
        predicted = np.ones(700)
        predicted[all_pass] = 0
        acc = np.sum(predicted == labels) / 700
        print("Detection accuracy with all defense on - all: ", acc)
        print("Detection f1 with all defense on - all: ", f1_score(labels, predicted))

        print("\n==========================================================")
        all_pass, _ = self.operator.filter(self.operator.data.test_group_data[-300:700], thrs)
        all_on_acc, _, _, _, predicted = self.get_normal_acc(all_pass, 300)
        print('Num pass', len(all_pass))
        print("Classification accuracy with all defense on - 1,2,3: ", all_on_acc)
        print("Classification f1 with all defense on - 1,2,3: ",
              f1_score(np.argmax(data.test_group_pred_labels[-300:700], axis=1), predicted))
        labels = data.test_group_detc_labels[-300:700]
        predicted = np.ones(300)
        predicted[all_pass] = 0
        acc = np.sum(predicted == labels) / 300
        print("Detection accuracy with all defense on - 1,2,3: ", acc)
        print("Detection f1 with all defense on - 1,2,3: ", f1_score(labels, predicted))

        print("\n==========================================================")
        all_pass, _ = self.operator.filter(self.operator.data.test_group_data[0:100], thrs)
        all_on_acc, _, _, _, predicted = self.get_normal_acc(all_pass, 100)
        print('Num pass', len(all_pass))
        print("Classification accuracy with all defense on - 4: ", all_on_acc)
        print("Classification f1 with all defense on - 4: ",
              f1_score(np.argmax(data.test_group_pred_labels[0:100], axis=1), predicted))
        labels = data.test_group_detc_labels[0:100]
        predicted = np.ones(100)
        predicted[all_pass] = 0
        acc = np.sum(predicted == labels) / 100
        print("Detection accuracy with all defense on - 4: ", acc)
        print("Detection f1 with all defense on - 4: ", f1_score(labels, predicted))

        print("\n==========================================================")
        all_pass, _ = self.operator.filter(self.operator.data.test_group_data[100:200], thrs)
        all_on_acc, _, _, _, predicted = self.get_normal_acc(all_pass, 100)
        print('Num pass', len(all_pass))
        print("Classification accuracy with all defense on - 5: ", all_on_acc)
        print("Classification f1 with all defense on - 5: ",
              f1_score(np.argmax(data.test_group_pred_labels[100:200], axis=1), predicted))
        labels = data.test_group_detc_labels[100:200]
        predicted = np.ones(100)
        predicted[all_pass] = 0
        acc = np.sum(predicted == labels) / 100
        print("Detection accuracy with all defense on - 5: ", acc)
        print("Detection f1 with all defense on - 5: ", f1_score(labels, predicted))

        print("\n==========================================================")
        all_pass, _ = self.operator.filter(self.operator.data.test_group_data[200:300], thrs)
        all_on_acc, _, _, _, predicted = self.get_normal_acc(all_pass, 100)
        print('Num pass', len(all_pass))
        print("Classification accuracy with all defense on - 6: ", all_on_acc)
        print("Classification f1 with all defense on - 6: ",
              f1_score(np.argmax(data.test_group_pred_labels[200:300], axis=1), predicted))
        labels = data.test_group_detc_labels[200:300]
        predicted = np.ones(100)
        predicted[all_pass] = 0
        acc = np.sum(predicted == labels) / 100
        print("Detection accuracy with all defense on - 6: ", acc)
        print("Detection f1 with all defense on - 6: ", f1_score(labels, predicted))

        print("\n==========================================================")
        all_pass, _ = self.operator.filter(self.operator.data.test_group_data[300:400], thrs)
        all_on_acc, _, _, _, predicted = self.get_normal_acc(all_pass, 100)
        print('Num pass', len(all_pass))
        print("Classification accuracy with all defense on - 7: ", all_on_acc)
        print("Classification f1 with all defense on - 7: ",
              f1_score(np.argmax(data.test_group_pred_labels[300:400], axis=1), predicted))
        labels = data.test_group_detc_labels[300:400]
        predicted = np.ones(100)
        predicted[all_pass] = 0
        acc = np.sum(predicted == labels) / 100
        print("Detection accuracy with all defense on - 7: ", acc)
        print("Detection f1 with all defense on - 7: ", f1_score(labels, predicted))
