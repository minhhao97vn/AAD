...
def main():
    ...
    classifier.fit(x_test_shuffle.reshape((-1, 28, 28, 1)), y_test_shuffle[:, 0:-1], batch_size=64, nb_epochs=10)

    # Calling poisoning defence:
    defense = SpectralSignatureDefense(classifier, x_test.reshape((-1, 28, 28, 1)),
                                       y_test[:, 0:-1],
                                       expected_pp_poison=0.5, batch_size=64, eps_multiplier=0.1)
