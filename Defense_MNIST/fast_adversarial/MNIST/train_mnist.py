...
def main():
    ...
    train_data = np.load('../../data/AAD_MNIST_train.npz')
    x_train = (train_data['X_all'] * 255).astype(int)
    y_train = train_data['y_all']
    mnist_train = TensorDataset(Tensor(x_train[y_train[:, -1] == 0]),
                                Tensor(np.argmax(y_train[y_train[:, -1] == 0][:, 0:-1], axis=1)))
