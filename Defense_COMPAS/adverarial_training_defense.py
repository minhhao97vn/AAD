...
def main():
    ...
    y_train_group_cate = [to_categorical(group[:, 0]) for group in y_train_group]
    ...
    defense.fit(x_groups=X_train_group, y_groups=y_train_group_cate, nb_epochs=100)

    # Evaluate the classifier on the test set
    preds = np.argmax(defense.predict(X_test), axis=1)
    acc = np.sum(preds == np.argmax(to_categorical(y_test[:, 0]), axis=1)) / y_test[:, 0].shape[0]
    print("\nTest accuracy: %.2f%%" % (acc * 100))
