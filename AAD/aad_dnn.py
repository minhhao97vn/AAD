...
from data_processor import COMPASProcessor
...
train_loader, test_loader = COMPASProcessor().poisoned_loader_shift(batch_size=batch_size)
...
class MLP_LR_context_detection_regularization(nn.Module):
    ...
    def forward(self, x):
        ...
        prediction_vector_x = torch.cat((context_vector, x), dim=1)
        ...
        detection_vector_x = torch.cat((context_vector, x), dim=1)
...
for epoch in range(num_epochs):
    ...
    for index in group_ids:
        ...
        loss_context = criterion_context(torch.cat((1 - y_pre, y_pre), dim=1),
                                         torch.cat((1 - y_context, y_context), dim=1))
