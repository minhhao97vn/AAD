    ...
    if False:
        ...
        with torch.no_grad():
            ...
            for images, labels in train_loader:
                ...
                test_prediction_labels.append(Tensor.cpu(labels).numpy())
                test_prediction_predicted.append(np.eye(10)[Tensor.cpu(predicted).numpy()])
            ...
            print('F1 score of the training example: {} %'.format(
                f1_score(np.concatenate(test_prediction_labels, axis=0), np.concatenate(test_prediction_predicted, axis=0),
                         average='macro')))
            ...
            print('F1 score of the test example: {} %'.format(
                f1_score(np.concatenate(test_prediction_labels, axis=0), np.concatenate(test_prediction_predicted, axis=0),
                         average='macro')))
        model.train()
end_time = time.time()
...
# Each divided group
test_data_set = np.load('AAD_MNIST_test.npz')
...
with torch.no_grad():
    ...
    for group in group_test_data[0:4]:
        ...
        outputs = model(images)
        predicted = torch.argmax(outputs, dim=1)
        labels_class = torch.argmax(labels_pred, dim=1)

        correct_attack += (predicted.eq(labels_class)).sum().item()
